...
const stripExerciseMedia = (exercise: Exercise) => ({
  id: exercise.id,
  name: exercise.name,
...
export class LocalStorageWorkoutRepository implements IWorkoutRepository {
  ...
  async getExercises(): Promise<Exercise[]> {
    this.ensureSchemaUpdated();
    let saved = this.getItem<Exercise[]>(STORAGE_KEYS.EXERCISES, []);
    if (!saved || saved.length === 0) {
      this.setItem(STORAGE_KEYS.EXERCISES, DEFAULT_EXERCISES);
      return DEFAULT_EXERCISES;
    }

    saved = this.syncSavedDefaultExerciseMedia(saved);
  ...
  private syncSavedDefaultExerciseMedia(exercises: Exercise[]): Exercise[] {
    ...
    const synced = exercises.map((exercise) => {
      ...
      if (!defaultExercise.imageUrl && !defaultExercise.videoUrl && !defaultExercise.videoEmbedUrl) {
        return exercise;
      }

      const sameSeedContent =
        JSON.stringify(stripExerciseMedia(exercise)) ===
        JSON.stringify(stripExerciseMedia(defaultExercise));

      if (!sameSeedContent) {
        return exercise;
      }

      const nextImageUrl = exercise.imageUrl || defaultExercise.imageUrl;
      const nextVideoUrl = exercise.videoUrl || defaultExercise.videoUrl;
      const nextVideoEmbedUrl = exercise.videoEmbedUrl || defaultExercise.videoEmbedUrl;

      if (
        nextImageUrl === exercise.imageUrl &&
        nextVideoUrl === exercise.videoUrl &&
        nextVideoEmbedUrl === exercise.videoEmbedUrl
      ) {
        return exercise;
      }
      ...
      return {
        ...
        ...(nextVideoEmbedUrl ? { videoEmbedUrl: nextVideoEmbedUrl } : {}),
      };
    });

    if (changed) {
      this.setItem(STORAGE_KEYS.EXERCISES, synced);
    }
